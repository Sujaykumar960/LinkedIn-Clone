    ...
    <div className='px-5 xl:px-50 py-9 flex flex-col gap-5 w-full mt-5 bg-gray-100'>
        <div className='py-4 px-10 border-1 border-gray-400 w-full flex justify-between my-5 text-xl bg-white rounded-xl'>
            ...
            <div className='flex gap-3'>
                <button onClick={handleFriends} className={`p-1 border-1 rounded-lg border-gray-300 cursor-pointer ${text==="Catch Up with Friends"?'bg-blue-800 text-white':''}`}>Friends</button>
                <button onClick={handlePending} className={`p-1 border-1 rounded-lg border-gray-300 cursor-pointer ${text==="Pending Request"?'bg-blue-800 text-white':''}`}>Pending Request</button>
            </div>
        </div>
